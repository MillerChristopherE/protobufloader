...
public class ProtobufSerde3 extends AbstractDeserializer { // implements SerDe {
    ...
    static class ProtoLazyRoot extends ProtoRoot
    {
        ...
        public Object getProtoValue()
        {
            if(length != -1)
            {
                try
                {
                    msg = serde.getMessageFromInputBytes(buffer, length);
                }
    ...
    protected Message getMessageFromProtoBytes(byte[] inputBytes, int len) throws SerDeException, java.io.IOException {
        //msgSize = len;
        Message.Builder builder = newBuilder();
        try
        {
            Message msg = builder.mergeFrom(inputBytes, 0, len).build();
    ...
    protected Message getMessageFromInputBytes(byte[] inputBytes, int len) throws SerDeException, java.io.IOException {
        return getMessageFromProtoBytes(inputBytes, len);
    }
    ...
    protected Object getRecordFromInput(byte[] inputBytes, int len) throws SerDeException, java.io.IOException
    {
        ...
        */
        ...
        Message msg = getMessageFromInputBytes(inputBytes, len);
        if(msg == null)
        {
            return null;
        }
    ...
    public Object deserialize(Writable inputBlob) throws SerDeException {
        BytesWritable value = (BytesWritable)inputBlob;
